...
def spinning_dust(nu, Asd=1.):
    ame_file = np.load('/burg/home/as6131/CMB_dist_instrument/sd_foregrounds_optimize/templates/ame.npy').astype(ndp)
    ame_nu = ame_file[0]
    ame_I = ame_file[1]
...
def co_rad(nu, Aco=1.):
    x = np.load('/burg/home/as6131/CMB_dist_instrument/sd_foregrounds_optimize/templates/co_arrays.npy').astype(ndp)
    freqs = x[0]
    co = x[1]
